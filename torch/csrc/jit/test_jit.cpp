...
static void fusionTests() {
  ...
  auto testSimple = [&] {
    ...
    auto o = at::zeros({3,4}, at::CUDA(at::kFloat));
  ...
  auto testOne = [&](int ti, int tj, int toi, int toj) {
    ...
    for(size_t i = 0; i < graph.outputs().size(); i++) {
      ...
      outputs.push_back(at::zeros(dims, at::CUDA(at::kFloat)).transpose(toi,toj));
    }
  ...
  auto testConcat = [&](int dim) {
    ...
    auto o = at::zeros({3,4,5}, at::CUDA(at::kFloat));

    auto o_r = a*b;
    auto o2_r = at::cat({a, o_r}, dim);
    auto o2 = at::zeros(o2_r.sizes(), at::CUDA(at::kFloat));
