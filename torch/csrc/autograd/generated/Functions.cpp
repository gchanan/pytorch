...
variable_list ThAddmmBackward::apply(variable_list&& grads) {
  IndexRangeGenerator gen;
  auto self_ix = gen.range(1);
...
variable_list SNativeAddmmBackward::apply(variable_list&& grads) {
  IndexRangeGenerator gen;
  auto self_ix = gen.range(1);
  auto mat1_ix = gen.range(1);
  auto mat2_ix = gen.range(1);
...
variable_list GesvSingleBackward::apply(variable_list&& grads) {
  IndexRangeGenerator gen;
  auto self_ix = gen.range(1);
  auto A_ix = gen.range(1);
  variable_list grad_inputs(gen.size());
...
variable_list GesvHelperBackward::apply(variable_list&& grads) {
  IndexRangeGenerator gen;
  auto self_ix = gen.range(1);
  auto A_ix = gen.range(1);
  variable_list grad_inputs(gen.size());
...
variable_list InverseBackward::apply(variable_list&& grads) {
  ...
  auto result = result_.unpack(shared_from_this());
  if (should_compute_output({ self_ix })) {
    auto grad_result = -at::mm(result.t(), at::mm(grad, result.t()));
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
...
variable_list PotrfBackward::apply(variable_list&& grads) {
  IndexRangeGenerator gen;
  auto self_ix = gen.range(1);
  variable_list grad_inputs(gen.size());
  auto& grad = grads[0];
  auto result = result_.unpack(shared_from_this());
  if (should_compute_output({ self_ix })) {
    auto grad_result = potrf_backward(grad, upper, result);
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
