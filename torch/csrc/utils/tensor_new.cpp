...
Tensor tensor_ctor(const Type& type, PyObject* args, PyObject* kwargs) {
  ...
  if (r.idx == 0) {
    ...
    return set_requires_grad(internal_new_from_data(typeWithDefault(r, 1, type), r.deviceInt64(2), r.pyobject(0), true, true, type_inference), r.toBool(3));
  }
...
Tensor new_tensor(const Type& type, PyObject* args, PyObject* kwargs) {
  ...
  auto r = parser.parse(args, kwargs, parsed_args);
  if (r.idx == 0) {
    return set_requires_grad(new_from_data_copy(typeWithDefault(r, 1, type), r.deviceInt64(2), r.pyobject(0)), r.toBool(3));
  }
