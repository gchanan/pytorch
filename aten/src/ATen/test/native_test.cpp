...
void test_matmul(const Type& T) {
  ...
  // just make sure ASSERT_ALLCLOSE fails
  auto d5ones = T.ones({3, 2, 4, 2, 3});
  bool passed = false;
  try {
    std::cerr << "doing d5ones" << std::endl;
    std::cerr << d5ones.matmul(d1) << std::endl;
    std::cerr << "doing d5 matmul" << std::endl;
    std::cerr << d5.matmul(d1) << std::endl;
...
int main() {
  std::cerr << "native test" << std::endl;
  std::cout << "native test" << std::endl;
  Type & T = CPU(kFloat);
  ...
  // matmul
  {
    std::cerr << "test matmul" << std::endl;
    test_matmul(T);
    if(at::hasCUDA()) {
      std::cerr << "cuda matmul" << std::endl;
      test_matmul(CUDA(kFloat));
    }
  }
