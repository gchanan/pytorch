...
/*Tensor standard_gamma_grad(const Tensor& self, const Tensor& alpha) {
  ...
  dispatch_cpu_floating_point3<myF2>(5);
  //dispatch_cpu_floating_point<tensor_apply3>(ret, self, alpha, op);
  
  //dispatch_cpu_floating_point<myF>(ret.type(), ret, self, alpha);
...
struct AnotherF {
  static void apply(Tensor& ret, const Tensor& self, const Tensor& alpha) {
    StandardGammaGradOp<T> op;
    tensor_apply3<T, StandardGammaGradOp<T>>(ret, self, alpha, op);
  }
};
