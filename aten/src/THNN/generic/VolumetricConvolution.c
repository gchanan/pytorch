          ...
          int pH)
{
  ...
  THNN_ARGCHECK(gradOutput->new_dim() == 4 || gradOutput->new_dim() == 5, 3,
		gradOutput,
		"4D or 5D (batch mode) tensor expected for gradOutput, but got: %s");

  int dimPlane = 0;
  if (gradOutput->new_dim() == 5)
  {
    dimPlane++;
  }
  ...
  if (gradOutput->new_dim() == 4) /* non-batch mode */
  {
          ...
          accreal scale_)
{
  ...
  THNN_ARGCHECK(gradWeight->new_dim() == 5, 4, gradWeight,
		"5D (nOutputPlane x nInputPlane x kT x kH x kW) tensor "
		"expected for gradWeight, but got: %s");

  int nOutputPlane = (int)gradWeight->size[0];
  if (gradBias) {
    THArgCheck(gradBias->dim() == 1 && gradBias->size[0] == nOutputPlane, 5,
