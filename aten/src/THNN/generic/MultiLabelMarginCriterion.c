          ...
          bool reduce)
{
  ...
  THArgCheck((input->dim() == 1) || (input->dim() == 2), 2,
  ...
  if (input->dim() == 1)
  {
    nframe = 1;
    dim = input->size[0];
    THArgCheck((target->dim() == 1) && (target->size[0] == dim), 3,
  ...
  else
  {
    nframe = input->size[0];
    dim = input->size[1];
    THArgCheck((target->dim() == 2) && (target->size[0] == nframe)
          ...
          bool reduce)
{
  ...
  if (input->dim() == 1)
  {
    ...
    THArgCheck((target->dim() == 1) && (target->size[0] == dim), 3,
	       "inconsistent target size");
    THArgCheck((isTarget->dim() == 1) && (isTarget->size[0] == dim), 3,
