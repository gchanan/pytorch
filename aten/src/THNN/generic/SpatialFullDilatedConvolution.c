...
static inline void THNN_(SpatialFullDilatedConvolution_shapeCheck)(
  ...
  if (weight != NULL) {
    AT_CHECK(!weight->is_empty() && (weight->dim() == 2 || weight->dim() == 4), 5, weight,
             "non-empty 2D or 4D weight tensor expected, but got size: ", weight->sizes());
    if (bias != NULL) {
  ...
  AT_CHECK(!input->is_empty() && (ndim == 3 || ndim == 4), 2, input,
           "3D or 4D input tensor expected but got size: ", input->sizes());
