...
\
  ...
  { \
    ...
    TENSOR##_size = 1; \
    TENSOR##_stride = 1; \
    for(TENSOR##_i = TENSOR->dim()-1; TENSOR##_i >= 0; TENSOR##_i--) { \
    ...
    if (!TENSOR##_contiguous) { \
      ...
      TENSOR##_dim = 1; \
      for(TENSOR##_i = TENSOR->dim()-2; TENSOR##_i >= 0; TENSOR##_i--) \
      { \
      ...
      TENSOR##_dimOffset = (DIM == TENSOR->dim()-1) ? &TENSOR##_i : &TENSOR##_counter[DIM]; \
      TENSOR##_sizes[TH_TENSOR_dim_index] = TENSOR->size[TENSOR->dim()-1]; \
      TENSOR##_strides[TH_TENSOR_dim_index] = TENSOR->stride[TENSOR->dim()-1]; \
      ...
      for(TENSOR##_i = TENSOR->dim()-2; TENSOR##_i >= 0; --TENSOR##_i) { \
        if (TENSOR->stride[TENSOR##_i] == TENSOR->stride[TENSOR##_i+1] * TENSOR->size[TENSOR##_i+1] && TENSOR##_i != DIM && TENSOR##_i+1 != DIM) { \
          ...
          if (DIM != TENSOR->dim()-1 && TENSOR##_i < DIM) \
            TENSOR##_dimOffset--; \
        } else { \
          --TH_TENSOR_dim_index; \
