  ...
  if(storage->flag & TH_STORAGE_RESIZABLE)
  {
    if(storage->allocator->realloc == NULL) {
      ...
      if (size == 0) {
        ...
        storage->data_ptr = static_cast<real*>(storage->allocator->malloc(
      ...
      storage->data_ptr = static_cast<real*>(storage->allocator->realloc(
              storage->allocatorContext,
