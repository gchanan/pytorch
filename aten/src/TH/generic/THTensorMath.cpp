...
void THTensor_(indexSelect)(THTensor *tensor, THTensor *src, int dim, THLongTensor *index)
{
  ...
  THArgCheck(index->dim() <= 1, 3, "Index is supposed to be an empty tensor or a vector");
...
real THTensor_(minall)(THTensor *tensor)
{
  ...
  THArgCheck(tensor->dim() > 0, 1, "tensor must have one dimension");
...
void THTensor_(catArray)(THTensor *result, THTensor **inputs, int numInputs, int dimension)
{
  ...
  if (cat_dimension == 0 && allContiguous) {
    ...
    for (int j = 0; j < numInputs; j++) {
      if (inputs[j]->dim()) {
        int64_t dimSize = cat_dimension < inputs[j]->dim() ? inputs[j]->size[cat_dimension] : 1;
...
{
  THArgCheck(tensor->dim() > 0, 1, "empty Tensor");
