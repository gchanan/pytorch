...
def create_python_bindings(python_functions, has_self, is_module=False):
    ...
    def emit_single_dispatch(declaration, out_idx, base_env):
        ...
        _python_binding_args = declaration.get('python_binding_arguments', [])
        has_dtype = False
        if len(_python_binding_args) > 1:
            formal_args.append('const at::Type& type')
            dtype_idx = arg_idx if out_idx is None else out_idx + 2
            actuals.append('r.dtype({})'.format(dtype_idx))
            has_dtype = True
        ...
        requires_grad = None
        python_binding_arg_count = len(declaration.get('python_binding_arguments', []))
